...
namespace LinnworksAPI
{
    public static class PrintServiceMethods
    {
        ...
        {
            return Newtonsoft.Json.JsonConvert.DeserializeObject<CreatePDFResult>(Factory.GetResponse("PrintService/CreatePDFfromJobForceTemplate", "templateType=" + templateType + "&IDs=" + Factory.SerializeAndUrlEscape(IDs) + "&templateID=" + Factory.SerializeAndUrlEscape(templateID) + "&parameters=" + Factory.SerializeAndUrlEscape(parameters) + "&printerName=" + printerName + "", ApiToken, ApiServer), new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
        }
